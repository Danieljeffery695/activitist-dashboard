...
export default function Header() {
    ...
    //IntersectionObservers start here 
      ...
      const {ref: myRef1, inView: elementInPlace1} = Observer(0, "0px", true);
    ...
    <div className="header-div flex my-3 320:h-800 320:pt-25 320:flex-col 320:text-center 410:h-800 850:text-left 850:flex-row">
      <div ref={myRef} className={`md:flex-row flex-1 h-full 850:pt-34
        ${elementInPlace ? 'translate-y-0 opacity-100' : 'translate-y-[50%] opacity-0'} transition-all transition-discrete delay-500 duration-500 ease-in-out`}>
      ...
      <div ref={myRef1} className="320:h-300 540:h-400 md:flex-1 md:h-full relative">
        <Image src="/image1.png" alt="Image" fill 
        className={`object-contain ${elementInPlace1 ? 'translate-y-0 opacity-100' : 'translate-y-[50%] opacity-0'} transition-all transition-discrete delay-700 duration-700 ease-in-out`}/>
      </div>
    </div>
    //  Header ends here
    );
